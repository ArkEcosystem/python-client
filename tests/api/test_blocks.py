...
def test_all_calls_correct_url_with_default_params():
    ...
    assert responses.calls[0].request.url == 'http://127.0.0.1:4002/api/blocks?limit=100'
...
def test_all_calls_correct_url_with_passed_in_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url.startswith('http://127.0.0.1:4002/api/blocks?')
...
def test_get_calls_correct_url():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == 'http://127.0.0.1:4002/api/blocks/12345'
...
def test_first_calls_correct_url():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == 'http://127.0.0.1:4002/api/blocks/first'
